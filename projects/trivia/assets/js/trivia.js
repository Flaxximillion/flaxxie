...
function difficulty(category) {
    ...
    for (let i = 0; i < difficulties.length; i++) {
        $(".gameContainer").append($("<button>")
            .prop("type", "input")
            .text(difficulties[i])
            .addClass("linedButton difficulty")
            .val(difficulties[i])
            .click(function () {
                $("body").effect("drop", {"direction": "right"}, 1000)
                    .promise()
                    .done(function () {
...
function displayQuestion(question) {
    ...
    $(questionDiv).prepend($("<div>").text(currQuestion)
        .addClass("question"));

    for (let i = 0; i < answers.length; i++) {
        $(answersDiv).append($("<button>")
            .text(parseHTML(answers[i]))
            .prop("type", "input")
            .addClass("answers linedButton")
            .val(answers[i]));
    }
...
function checkAnswer(userValue) {
    ...
    setTimeout(function () {
        $("body").effect("drop", {"direction": "right"}, 500)
            .promise()
            .done(function () {
                numOfQuestions++;
                currTime = 30;
...
function gameEnd() {
    ...
    $("#gameText").html($("<div>")
        .addClass("final")
        .text("Final score: " + numberOfRightAnswers + "  out of " + (numOfQuestions)));

    $("#gameText").append($("<button>")
        .addClass("linedButton playAgain")
        .text("Play again!")
        .click(function () {
            $("body").effect("drop", {"direction": "right"}, 500, function(){
                init();
            });
        }));
}